...
export async function initializePlayer(client) {
  try {
    ...
    player.on('queueEnd', (queue) => {
      const channel = queue.metadata?.textChannel;
      if (channel) {
        channel.send({ embeds: [{ color: 0x2f3136, title: 'Queue Ended', description: 'No more songs.' }] }).catch(() => {});
      }
    });
...
export function getQueueInfo(guildId) {
  ...
  return { currentTrack: queue.currentTrack, tracks: queue.tracks, size: queue.size, isPaused: queue.node.isPaused(), volume: queue.node.volume, connection: !!queue.connection };
}
